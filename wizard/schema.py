...
def create_table_schema_from_data(
    table_name: str, sample_data: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    columns.append({"name": "id", "type": "SERIAL PRIMARY KEY"})

    # Infer columns from sample data
    for key, value in sample_data.items():
        if key != "id":  # Skip if user provided ID
            postgres_type = infer_postgres_type(value)
            columns.append({"name": key, "type": postgres_type})

    return {"table_name": table_name, "columns": columns, "created_from_data": True}
